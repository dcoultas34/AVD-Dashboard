# Top User Processes by CPU/RAM
# Same as top-processes.ps1 but excludes SYSTEM, LOCAL SERVICE and NETWORK SERVICE accounts.
# Shows top 15 by CPU %, then top 15 by RAM usage.
...
# Get all user processes (exclude system accounts)
$allProcs = Get-Process -IncludeUserName -ErrorAction SilentlyContinue |
    Where-Object { $_.UserName -and $_.UserName -notmatch "NT AUTHORITY|Window Manager" }
...
# --- Top 15 by RAM (directly from Get-Process, includes all user processes) ---
