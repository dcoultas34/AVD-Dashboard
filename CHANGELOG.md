...
## 2026-07-11.5
- **Friendlier sign-in dialogs.** Cancelling the Azure sign-in now shows a calm "Sign-in was cancelled" notice (and exits cleanly) instead of a raw exception dump with a red error icon. All sign-in and subscription errors now use a WPF dialog styled like the update prompt, with a plain-English message up top and the technical detail in a small scrollable box.

## 2026-07-11.4
- **Update backups now go into a `backup` folder** (mirroring the folder structure) instead of leaving `.bak` files scattered next to the live ones. The folder always holds the pre-update version of every file the updater has ever replaced; rollback on a failed update restores from there. Any old `<file>.bak` files from previous updates can be deleted.

## 2026-07-11.3
- **Fixed a perpetual "Update Available" prompt on fresh downloads.** The 2026-07-11.2 line-ending normalization was meant to keep `.cmd` files CRLF but accidentally committed them as LF, while the manifest was hashed from the CRLF working tree - so downloads flagged all launchers as changed and applying the update never converged. The `.cmd` files are now committed with CRLF as intended, and the manifest generator verifies its hashes against the staged git blobs and warns on any divergence so this class of mismatch can't ship silently again.

## 2026-07-11.2
- **Auto-update hardening and polish** (first live release of the feature shipped earlier today shook out several gaps):
  - Update checks are now **commit-pinned**: the current HEAD sha of `main` is resolved via the GitHub API (not behind the raw-content CDN cache), and the manifest plus all files are fetched from immutable `raw.githubusercontent.com/<repo>/<sha>/` URLs. This eliminates the ~5-minute stale-manifest window after a push and guarantees the manifest and files come from the same commit. Falls back to the branch URL if the API is unreachable.
  - **Automatic rollback**: if replacing files fails partway (previously left a silent partial update while claiming "nothing was changed"), everything already replaced is restored from the `.bak` copies and the failure dialog reports accurately.
  - **Locked-file handling**: files the running process has loaded (`lib\*.dll`) are staged as `<file>.new` and swapped in by `Complete-DashboardPendingUpdate` at the very top of the next launch, before anything locks them. Both apps now dot-source `scripts\update-check.ps1` near the top of the script for this reason.
  - **Line-ending normalization**: added `.gitattributes` (`* -text`) and converted tracked text files to LF (`.cmd` stays CRLF), so clones made with `core.autocrlf=true` don't hash every file differently from GitHub's blobs and report all files as changed.
  - Update prompt now shows a **"What's new"** changelog excerpt (new `notes` field in the manifest), and says "An updated build is available" instead of a confusing identical Current/New pair when a hotfix ships without a version bump. A small progress window shows per-file download progress instead of a frozen UI, and the post-update relaunch uses a hidden console window to match the `.cmd` launchers.
  - **Manifest can no longer go stale**: a committed pre-commit hook (`.githooks\pre-commit`, enabled via `git config core.hooksPath .githooks`) regenerates `update-manifest.json` on every commit.

## 2026-07-11
- **Added manifest-based auto-update** for the dashboard and Profile Tools. On launch, before the Azure sign-in prompt, each app fetches `update-manifest.json` from this repo (`raw.githubusercontent.com`) and compares the SHA-256 hash of every listed file against the local copy — only files that actually changed are downloaded, not a full re-download every release. If changes are found, a prompt lists them and offers Update / Not now; accepting stages and validates every changed file (PowerShell/XAML parse checks, hash re-verification) before backing up (`<file>.bak`) and applying them all-or-nothing, then relaunches with the original command-line arguments. A manual "Check for Updates" button was added to both apps' About dialogs. New `-SkipUpdateCheck` switch disables the automatic check. The check fails completely silently (log line only, no dialog) whenever the manifest can't be reached — the expected steady state while this repo is private.
  - New: `scripts/update-check.ps1` (the shared engine — fetch/diff/prompt/apply/relaunch), `tools/New-UpdateManifest.ps1` (maintainer tool that regenerates `update-manifest.json` from `$ScriptVersion` + per-file hashes), `update-manifest.json`, `AUTO-UPDATE.md` (schema + release workflow docs).
  - Updated the stale `virtualwebber/AVD-Dashboard` GitHub links (About-dialog hyperlinks, author comments) in both scripts to point at this repo, `AVD-Dashboard---Dev`, which is the actual update source.
- Bumped `$ScriptVersion` in both `avd-live-dashboard.ps1` and `profile-tools.ps1` to match this release.

## 2026-07-08
- **Authentication: removed the `Az.Accounts` module dependency**, replaced with bundled MSAL.NET (`lib/Microsoft.Identity.Client.dll` + `lib/Microsoft.IdentityModel.Abstractions.dll`) called directly via `Add-Type`. No `Install-Module` step is required for the dashboard or Profile Tools. `Connect-AzureDashboard` (`scripts/connect-azure.ps1`) now builds an MSAL `PublicClientApplication` using the well-known Azure PowerShell client ID and returns a context object shaped identically to the old `Get-AzContext` output (`.Subscription`, `.Account`, `.Tenant`), so no caller changes were needed elsewhere.
  - Interactive browser and device code sign-in both go through MSAL's native flows. `-UseExistingContext` now attempts silent token acquisition from a persisted MSAL cache (`%APPDATA%\AVDDashboard\token-cache.bin`, DPAPI-encrypted) before falling back to interactive sign-in.
  - `Get-ArmToken` / `Get-LawToken` (`scripts/rest-api-helpers.ps1`) call `AcquireTokenSilent` directly instead of `Get-AzAccessToken`; the manual token cache hashtable was removed since MSAL handles caching and expiry internally.
  - `Resolve-MfaChallenge` (Conditional Access step-up) was rewritten from a ~280-line child-`powershell.exe`-process design (which had to `Import-Module Az.Accounts` in a separate process to get a usable HTTP pipeline) down to ~90 lines that call MSAL's `AcquireTokenSilent`/`AcquireTokenInteractive` with `.WithClaims()` directly on the main thread.
  - The MSAL token-cache persistence callbacks (`SetBeforeAccess`/`SetAfterAccess`) are implemented as a compiled C# helper class (`AvdMsalCache`, via `Add-Type -TypeDefinition`) rather than PowerShell scriptblocks — MSAL invokes these callbacks on background threads with no PowerShell runspace, which threw "There is no Runspace available to run scripts in this thread" when a scriptblock was used.
  - Subscription switching no longer spawns a dedicated runspace to reimport a saved Az context; it now makes a single inline `Invoke-RestMethod` call to resolve the new subscription's display name.
  - **Service principal authentication was removed.** `-UseServicePrincipal` is still accepted as a parameter (so existing shortcuts/configs don't hard-error) but now shows a "Not Supported" dialog and exits. `scripts/check-permissions.ps1` was **not** migrated and is now the only script in the suite still requiring `Az.Accounts` — it retains full service principal support on its own separate auth path.
  - Full pre-migration implementation details and a step-by-step rollback guide are documented in `misc/az-accounts-reference.md`.
- Fixed two PowerShell 5.1 gotchas introduced during the MSAL migration: backtick line-continuation chaining into a leading `.Method()` call is a parse error in PowerShell (unlike C#) — all MSAL fluent-builder chains now use intermediate variables instead. Also removed em dashes and smart quotes from the new/edited files, which silently corrupt `.ps1` files under PowerShell 5.1's default encoding handling (see existing gotcha in `CLAUDE.md`).
- **`profile-tools.ps1`: fixed a leftover Az.Accounts module gate** — it still hard-blocked launch with a "Missing Modules" error if `Az.Accounts` wasn't installed, even though it no longer needs it (dot-sources the MSAL-based `connect-azure.ps1`). Replaced with the same MSAL DLL existence check used in `avd-live-dashboard.ps1`. Also updated its `.PARAMETER` docs (`UseExistingContext`, `UseServicePrincipal`) and bumped its own `$ScriptVersion`.
- Swept remaining code comments across `session-detail.ps1` and `tab-sessioninfo.ps1` that still described the old Az.Accounts/child-process MFA design or `Get-AzAccessToken` Graph token calls — the underlying code in `tab-sessioninfo.ps1` already called the generic (now MSAL-based) `Get-ArmToken` correctly and needed no functional change, only comment updates.
- Config editor (`scripts/edit-config.ps1`): `Import-ConfigFile` now distinguishes an empty/missing path, a missing file, an empty file, and an actual parse error, showing a specific message for each instead of a single generic ".NET exception" banner for all four. The **New** and **Browse / Open...** dialogs now default to the `config\` folder next to the script (falling back from the current config's folder when that path is invalid), rather than potentially opening in an unrelated location when the currently loaded config path is bad.
- Bumped `$ScriptVersion` to match this release.
...
## 2026-05-30b
- All in-app dialogs: replaced `[System.Windows.MessageBox]::Show()` with a new `Show-ThemedDialog` WPF helper that matches the dashboard's dark/light theme. Covers `avd-live-dashboard.ps1` (post-window-load errors and confirmations), `session-detail.ps1` (shadow, RDP, log-off, send message, export), `run-command.ps1`, `cost-lookup.ps1` (all timeout and error dialogs for Session Hosts, Azure Files, and Infrastructure tabs), and `tab-azuredevops.ps1`. Startup errors that fire before the WPF window exists remain as native MessageBox calls. The helper supports OK / OKCancel / YesNo button sets, Information / Warning / Error / Question icons (unicode glyphs with semantic colouring), automatic dark title bar, and live theme updates via the `Register-ThemedWindow` registry.
- Azure DevOps tab: themed the Running Pipelines popup and PAT Configuration dialog — both previously used hardcoded `Brushes.White` backgrounds and `FromRgb` colours; now use MergedDictionaries theme injection, `SetResourceReference` for all text/border colours, and styled buttons matching the rest of the dashboard.

## 2026-05-30
- Session Hosts tab: double-clicking a session host row now opens the Session Detail window filtered to that VM's sessions only. Passes the host pool name and RG so the correct AVD session list is fetched; a host-name filter (`$script:sdHostFilter`) is applied via `Update-SessionFilter` so only sessions on that specific VM are shown. The window title shows the VM name rather than the pool name to make the scope clear. Double-clicking from the Per Host Pool tab (PoolGrid) continues to open the full pool view unfiltered.
- Run Command / Session Detail theme: fixed popup windows not switching theme when the dark mode toggle is used. Each secondary window now registers with a `$script:_themedWindows` list via `Register-ThemedWindow` / `Unregister-ThemedWindow`; `Switch-DashboardTheme` iterates the list and pushes resource updates to all open windows. Separately, popup windows now read the current theme from disk at open time (based on `$script:DarkTheme`) rather than using the stale `$_themeContent` variable captured at startup.

## 2026-05-29
- Session Detail: fixed Run Command doing nothing when right-clicked — handler was accessing `HP RG`, a column that doesn't exist in the DataTable, causing a silent exception. Now uses the `Session Host` column (already the short VM name) and lets `Show-RunCommandPicker` resolve the resource group from `vmRgMap`.
- Per Host Pool / By Region: fixed `ConvertTo-DataTable` throwing when objects with different property sets appear in the same collection (e.g. an empty host pool row preceding a normal row). Schema is now built from the union of all objects' properties; rows that lack a given column are skipped safely.

## 2026-05-27
- Per Host Pool tab: fixed "Refresh error: Column 'VMs Drained' does not belong to table" when any host pool has no session hosts registered. The empty-pool row was missing the `VMs Drained` field, causing `ConvertTo-DataTable` to build the grid schema without that column; any non-empty pool row following it then threw when trying to set the missing column. Fixed by adding `"VMs Drained" = 0` to the empty-pool PSCustomObject.
